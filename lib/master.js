...
// Format log messages outside the console calls. External logger may not support the formatting.
...
cluster.on('online', function(worker) {
	...
	log.info(format('Worker %d %s (PID %d) online', worker.id, role, worker.process.pid));
});

cluster.on('listening', function(worker, address) {
	var env = workerEnv[worker.id],
		role = env? env.NODE_WORKER_ROLE : '';
	log.info(format('Worker %d %s (PID %d) listening on %s:%d', worker.id, role, worker.process.pid, address.address, address.port));
...
cluster.on('disconnect', function(worker) {
	var env = workerEnv[worker.id],
		role = env? env.NODE_WORKER_ROLE : '';
	log.warn(format('Worker %d %s (PID %d) disconnected', worker.id, role, worker.process.pid));
});
...
cluster.on('exit', function(worker, code, signal) {
	...
	if (worker.suicide === true || code === 0) {
		log.warn(format('Worker %d %s (PID %d) suicide%s', worker.id, role, worker.process.pid, respawn?'. Respawning cancelled.':''));
		respawn = false;
	} else {
		log.warn(format('Worker %d %s (PID %d) died (%s)%s', worker.id, role, worker.process.pid, signal||code, respawn?'. Respawning...':''));
	}
...
function spawn(config, dir, extLog) {
	if (extLog) {
		log = extLog;
	}

	if (!cluster.isMaster) {
		log.error('Unable to run Master in worker process.');
		return;
	}
